...
export class HospitalService {
  ...
  borrarHospital (id: string) {
    ...
    return this.http.delete(url).pipe(map(res => swal('Hospital Borrado', 'Eliminado correctamente', 'success')))
  }

  crearHospital (nombre: string) {
    let url = URL_SERVICIOS + '/hospital';
    url += '?token=' + this._usuarioService.token;
    return this.http.post(url, {nombre: nombre}).pipe(map((res: any) => res.hospital));
